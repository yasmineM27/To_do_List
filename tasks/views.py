...
def task_detail(request, pk):
    task = get_object_or_404(
        Task.objects.select_related('todo_list').prefetch_related('tags'),
        Q(pk=pk, todo_list__user=request.user) | Q(pk=pk, todo_list__shared_with=request.user)
    )
...
def edit_task(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if 'is_completed' in request.POST and not task.is_completed:
                task.mark_completed()
            elif 'is_completed' not in request.POST and task.is_completed:
                task.mark_incomplete()
            
            # Redirect based on where the task was edited from
            if 'from_kanban' in request.POST:
                return redirect('tasks:kanban_board', pk=task.todo_list.pk)
...
def delete_task(request, pk):
    ...
    return render(request, 'tasks/task/confirm_delete.html', {
        'task': task,
        'from_kanban': 'from_kanban' in request.GET
    })
...
def filter_by_tag(request, tag_pk):
    ...
    return render(request, 'tasks/task/filtered_tasks.html', {
        'tasks': tasks,
        'filter_type': 'tag',
...
def filter_week(request):
    ...
    return render(request, 'tasks/task/filtered_tasks.html', {
        'tasks': tasks,
        'filter_type': 'week',
        'filter_value': 'This Week\'s Tasks'
    })
...
def search_tasks(request):
    ...
    return render(request, 'tasks/task/search_results.html', {
        'tasks': tasks,
        'query': query
    })